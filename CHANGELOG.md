...
- **Notification foundation:** Career Twin activation records the authenticated email, matches have additive `notified` migration fields, and the offline-tested notification worker sends one digest per user and marks matches only after a successful injected delivery. A production Resend sender remains intentionally unconfigured until its credentials/domain are supplied.
