...
## Relevance-weighted selection (mandatory)
The CV must fit on ~2 pages, so cut low-value content by scoring each candidate bullet on: (a) relevance to THIS posting's requirements, (b) uniqueness in the document, (c) measurable impact. Cut lowest-scoring first. An older-role bullet that hits the posting's keywords survives ahead of a recent-role bullet that doesn't. Never pad with generic filler to fill space.

## Bullet rules
- Start with a strong verb; include numbers, tools, and outcomes wherever the profile provides them.
- Reorder so the most posting-relevant bullets come first within each role.
- Rewrite wording toward the posting's vocabulary ONLY where the underlying work genuinely matches (interview backtrack test). Mark anything stretchy with "(stretch — verify)".
