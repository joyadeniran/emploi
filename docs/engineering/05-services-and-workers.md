...
- **Inputs:** public, keyless board APIs — Greenhouse, Lever, Ashby, Workable, SmartRecruiters and **Workday** (`{tenant}.{wd}.myworkdayjobs.com/wday/cxs/...`, token `tenant:site` or `tenant:wdN:site`; the bank/corporate tier that Greenhouse/Lever never covers — SuccessFactors/Taleo need per-tenant credentials so they stay `career_page` placeholders) — plus **env-keyed aggregators**, each of which no-ops (logs a skip) when its key is unset so an unconfigured deploy stays quiet:
  - **Jooble** (`JOOBLE_API_KEY`, token `location:keywords`) — free-text location, the broad Nigeria volume lever; noisy, no real country field.
  - **Adzuna** (`ADZUNA_APP_ID`/`_KEY`, token `country:keywords`) — **no Nigeria endpoint** (use za/gb/us).
  - **jobdataapi.com** (`JOBDATAAPI_API_KEY`, token `CC` or `CC:keywords`) — structured feed with a **real ISO `country_code` filter**, so it tags Nigeria at the source; the normalizer trusts the feed's own country code and `has_remote` flag. Auth via `Authorization: Api-Key` header (the `_get_json_auth` seam). Seeded active.
  - **Techmap** "Jobs In Nigeria" (`TECHMAP_API_KEY`, header `x-api-key`, token country/location) — Nigeria-localised feed; normalizer is **defensive/provisional** (accepts either field-name set, a `{results}` wrapper or a bare array) and its seed rows ship `active=false` until the field mapping is confirmed against a live sample (`TECHMAP_BASE` overridable for the RapidAPI-vs-direct host split).

  Aggregators authenticate by header, not query key: the shared `_get_json_auth(url, headers)` seam (a GET counterpart to Jooble's `_post_json`) carries the key and never raises — a bad key/down endpoint becomes a logged `None` and the source no-ops. Source list comes from the `job_sources` DB table, seeded once from `data/job_sources.json`; add/disable sources via `/admin/job-sources` after that.
