...
- **Read→write lock (deadlock / 500 under concurrency).** Two sibling errors that the busy timeout does NOT cover, so SQLite raises them immediately: `SQLITE_BUSY_SNAPSHOT` (517, a WAL read snapshot upgrading to a write after another connection committed) and `SQLITE_BUSY` (5) on a *lock promotion* (a connection holding a SHARED lock — from a `SELECT`-then-write, or an open read cursor — writing while another connection holds the write lock). Unhandled, either is a user-facing 500 (e.g. `upsert_user` on every authenticated render racing a worker). Three defenses, in `db.py`:
  - `db.connect` uses `factory=_ResilientConnection`, whose `execute`/`executemany`/`commit` retry **every** lock error, `rollback()`-first (which releases the read snapshot / SHARED lock and **breaks the deadlock**), bounded by a wall-clock budget (`_LOCK_RETRY_BUDGET_S`, 20s) that MUST exceed the busy timeout (`_BUSY_TIMEOUT_S`, 5s) — otherwise a lock wait that expires never gets a retry and the breaking rollback never runs. On final give-up it rolls back before re-raising so a failed write can't poison the next op. Safe because a lock error fires at the transaction's first write, before anything is applied; `commit` never rolls back.
  - `upsert_user` (and any hot-path upsert) is a **single-statement `INSERT … ON CONFLICT DO UPDATE`**, never `SELECT`-then-write — a single write holds no prior read lock, so the read→write hazard is gone from the hottest path.
  - All DB access goes through `Connection.execute` (no raw cursors), so the override covers every caller.
- **Stale-transaction guard (defence in depth).** `api.main.get_conn()` rolls back any pooled connection it finds `in_transaction`, logging a warning. Every `db.py` writer commits before returning, so an open transaction on a reused thread-local connection means the previous request died mid-write — nothing committed is lost, and it caps the damage of any future swallowed-write bug at one request instead of the whole process.
- **A swallowed write failure MUST roll back.** A statement that fails has already opened a write transaction; catching the exception without `conn.rollback()` leaves that transaction open, and in WAL the connection then holds the single writer lock **for its entire life** — every later write in the process dies with `database is locked`, and no retry can help because nothing ever closes it. This actually shipped: `add_credits` swallowed the duplicate-`paystack_reference` `IntegrityError` on webhook replay (which Paystack does routinely) and wedged the API's write path. Any `except` around a write must roll back — see `add_credits`, `log_event`, `create_role_application`.
