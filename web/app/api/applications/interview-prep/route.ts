...
// Submission only — returns {job_id, status:"pending"} immediately. Interview
// prep is one Gemini call but goes through the same async job + poll path as
...
export async function POST(req: Request) {
  ...
  try {
    const data = await apiFetch<{ job_id: string; status: string }>("/applications/interview-prep", {
      method: "POST",
