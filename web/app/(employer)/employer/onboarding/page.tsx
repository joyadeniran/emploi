...
export default function EmployerOnboardingPage() {
  ...
  const [domainFromEmail, setDomainFromEmail] = useState(false);
  const [state, setState] = useState<"idle" | "busy">("idle");
  const [error, setError] = useState<{ blocked?: boolean; message: string } | null>(null);

  // Prefill from the signed-in identity once, on mount.
  useEffect(() => {
    const e = document.querySelector('meta[name="x-user-email"]')?.getAttribute("content") ?? "";
    setEmail(e);
    const fromEmail = workEmailDomain(e);
    if (fromEmail) {
      setDomain(fromEmail);
      setDomainFromEmail(true);
    }
  }, []);
